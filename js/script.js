 ...
 * Quản lý Chặng 1 (30 câu), chặng câu hỏi bổ sung (10 câu khó), đồng hồ 10 giây và điều khiển chiến thắng.
 */
...
const STAGE1_QUESTION_COUNT = 30;
const SUPPLEMENTAL_QUESTION_COUNT = 10;
const TOTAL_QUESTION_COUNT = STAGE1_QUESTION_COUNT + SUPPLEMENTAL_QUESTION_COUNT;

let supplementalRoundEnabled = false;
let stageQuestionLimit = STAGE1_QUESTION_COUNT;
...
function validateUniqueQuestions() {
    ...
    quizData.forEach((item) => {
        ...
        if (seen.has(key)) {
            duplicatedIds.push(`${seen.get(key)} & ${item.id}`);
        } else {
    ...
    if (duplicatedIds.length > 0) {
        console.warn(`Phat hien cau hoi trung lap: ${duplicatedIds.join(', ')}`);
    }
}

function shuffleRange(start, endExclusive) {
    const last = Math.min(endExclusive, quizData.length);
    for (let index = last - 1; index > start; index--) {
        const randomIndex = Math.floor(Math.random() * (index - start + 1)) + start;
        [quizData[index], quizData[randomIndex]] = [quizData[randomIndex], quizData[index]];
    }
}

function shuffleQuizData() {
    // Tron ngau nhien trong tung nhom de giu nguyen cau hoi bo sung khong bi tron vao chang 1.
    shuffleRange(0, STAGE1_QUESTION_COUNT);
    shuffleRange(STAGE1_QUESTION_COUNT, TOTAL_QUESTION_COUNT);
}

function getDisplayQuestionLimit() {
    if (supplementalRoundEnabled) {
        return Math.min(SUPPLEMENTAL_QUESTION_COUNT, Math.max(0, quizData.length - STAGE1_QUESTION_COUNT));
    }
    return Math.min(stageQuestionLimit, quizData.length);
}

function getDisplayQuestionNumber() {
    if (supplementalRoundEnabled) {
        return (currentQuestionIndex - STAGE1_QUESTION_COUNT) + 1;
    }
    return currentQuestionIndex + 1;
}

function refreshStageLabels() {
    const totalDisplay = getDisplayQuestionLimit();
    document.getElementById('total-q-count').textContent = totalDisplay;

    if (supplementalRoundEnabled) {
        document.getElementById('header-stage-pill').textContent = `Câu hỏi bổ sung: ${Math.min(SUPPLEMENTAL_QUESTION_COUNT, Math.max(0, quizData.length - STAGE1_QUESTION_COUNT))} câu khó`;
        document.getElementById('current-stage-badge').textContent = 'Vòng bổ sung';
    } else {
        document.getElementById('header-stage-pill').textContent = `Chặng 1: ${Math.min(STAGE1_QUESTION_COUNT, quizData.length)} Câu Hỏi Thử Thách`;
        document.getElementById('current-stage-badge').textContent = `Chặng 1 (${Math.min(STAGE1_QUESTION_COUNT, quizData.length)} câu)`;
    }
}
...
function startOfficialGame() {
    ...
    supplementalRoundEnabled = false;
    stageQuestionLimit = STAGE1_QUESTION_COUNT;
...
function loadQuestion(index) {
    if (index < 0 || index >= stageQuestionLimit || index >= quizData.length) return;

    currentQuestionIndex = index;
    isAnswerRevealed = false;
    stopQuestionAudio();

    const displayLimit = getDisplayQuestionLimit();
    const displayNumber = getDisplayQuestionNumber();
    const qData = quizData[currentQuestionIndex];
    document.getElementById('current-q-index').textContent = displayNumber;
    document.getElementById('total-q-count').textContent = displayLimit;

    const progress = (displayNumber / displayLimit) * 100;
    ...
    document.getElementById('question-text-box').textContent = `Câu ${displayNumber}: ${qData.question}`;
...
function handleNextQuestion() {
    if (currentQuestionIndex < stageQuestionLimit - 1 && currentQuestionIndex < quizData.length - 1) {
        loadQuestion(currentQuestionIndex + 1);
    } else {
        if (!supplementalRoundEnabled && quizData.length > STAGE1_QUESTION_COUNT) {
            showStage1CompletedScreen();
        }
    }
}

function handlePrevQuestion() {
    const minIndex = supplementalRoundEnabled ? STAGE1_QUESTION_COUNT : 0;
...
function triggerRescueScreen() {
    ...
    document.getElementById('current-stage-badge').textContent = 'Cứu trợ';
    setScreen('stage-clear-screen');
}

function startSupplementalRound() {
    supplementalRoundEnabled = true;
    stageQuestionLimit = Math.min(TOTAL_QUESTION_COUNT, quizData.length);
    refreshStageLabels();
    setScreen('quiz-screen');

    if (currentQuestionIndex < STAGE1_QUESTION_COUNT) {
        loadQuestion(STAGE1_QUESTION_COUNT);
    } else {
        loadQuestion(currentQuestionIndex);
    }
}

function resumeAfterRescue() {
    setScreen('quiz-screen');
    refreshStageLabels();
    resetCountdown();
}

function showStage1CompletedScreen() {
    if (countdownTimer) clearInterval(countdownTimer);
    stopCountdownAudio();
    stopQuestionAudio();
    setScreen('stage1-complete-screen');
}

function declareWinnerNow() {
    triggerVictory('Mời Quán quân tiến lên bục danh dự để nhận phần thưởng cao quý từ Ban Tổ Chức!');
}

function triggerVictory(customMessage) {
    if (countdownTimer) clearInterval(countdownTimer);
    stopCountdownAudio();
    stopQuestionAudio();

    const mainText = document.getElementById('victory-main-text');
    const subText = document.getElementById('victory-sub-text');
    if (mainText) {
        mainText.textContent = 'Nhiệt liệt chúc mừng Thí sinh Xuất Sắc Nhất đã chinh phục phần thi Rung Chuông Vàng!';
    }
    if (subText) {
        subText.textContent = customMessage || 'Mời Quán quân tiến lên bục danh dự để nhận phần thưởng cao quý từ Ban Tổ Chức!';
    }
